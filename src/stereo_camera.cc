...
void stereo_tracking(const std::shared_ptr<openvslam::config>& cfg,
    ...
    example::camera_hal hal(cfg->camera_->name_, cam_num);

    if (!hal.init(cfg->camera_->cols_, cfg->camera_->rows_, cfg->camera_->fps_)) {
    	spdlog::critical("Cannot initialise camera {}", cam_num);
    	SLAM.shutdown();
    	return;
    }
    ...
    std::thread thread([&]() {
        while (is_not_end) {
            ...
            is_not_end = hal.read_frame(frames[0], frames[1]);
            ...
            for (int i = 0; i < 2; i++) {
                if (scale != 1.0) {
                    cv::resize(frames_rectified[i], frames_rectified[i], cv::Size(), scale, scale, cv::INTER_LINEAR);
                }
            }
...
int main(int argc, char* argv[]) {
	...
	std::cerr << "setup_type must be stereo.";
